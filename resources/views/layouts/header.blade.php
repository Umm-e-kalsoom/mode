...
<div class="navbar-header">
    <a class="navbar-brand" href="<?php echo URL::to('/'); ?>">
        <b>
		    @if(file_exists('assets/images/'.$app_setting->app_logo) && !empty($app_setting->app_logo))
                <img src="{{asset('assets/images/').'/'.$app_setting->app_logo}}" alt="homepage" class="dark-logo" width="100%" id="logo_web">
            @else
                <img src="{{ asset('/images/logo2.png') }}" alt="homepage" class="dark-logo" width="60%" id="logo_web" style="margin-top:-15px">
            @endif

            @if(file_exists('assets/images/'.$app_setting->app_logo_small) && !empty($app_setting->app_logo_small))
